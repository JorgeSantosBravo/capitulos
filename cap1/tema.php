...
location.href="tema.php?id="+id+"&dec="+dec+"&orderby="+opcion.value;
...
var dec = getParameterByName('dec');
location.href="tema.php?id="+opcion.value+"&dec="+dec+"&orderby="+o;
...
location.href="tema.php?id="+id+"&dec="+opcion.value+"&orderby="+o;
...
<select id="tema" name="tema" onchange="cambiargen(this.id)">

<option value=""></option>
<?php
include "conexion.php";
$consulta=$miconexion->query("SELECT * FROM tema ORDER BY nombre_tema ASC"); 
while ($rows = $consulta->fetch_assoc()){
	if ($rows["id_tema"]!=8){
echo "<option value=".$rows["id_tema"].">".$rows["nombre_tema"]."</option>";
}
}
...
<?php
echo "<a class='borrar' href=tema.php?id=".$_GET["id"].">Borrar filtros </a>";
?>
</div>
<?php
...
echo "<div id='cuenta'>";
$con=$miconexion->query("SELECT nombre_tema,COUNT(*) as con FROM tema,peliculastemas WHERE tema.id_tema=peliculastemas.id_tema and tema.id_tema=".$_GET["id"]);
if ($rows = $con->fetch_assoc()) {
echo "Hay ".$rows["con"]." películas con el tema ".$rows["nombre_tema"];
}
echo "</div>";
...
echo "<div id='centro'>";
$cons="SELECT * FROM titulopelicula,peliculastemas,tema,titulo WHERE titulo.id_titulo=titulopelicula.id_pelicula and titulopelicula.id_pelicula=peliculastemas.id_pelicula and peliculastemas.id_tema=tema.id_tema and tema.id_tema=".$_GET["id"]." ".$plus." ORDER BY ".$orden;
$consulta=$miconexion->query($cons); 
while ($rows = $consulta->fetch_assoc()){
echo "<title>".$rows["nombre_tema"]."</title>";
